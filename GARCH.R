...
fit_garch_benchmark <- function(model_name, rugarch_model, solver = "hybrid") {
  spec <- ugarchspec(
    variance.model = list(
      model = rugarch_model,
      garchOrder = c(1, 1),
...
benchmark_results$sGARCH <- fit_garch_benchmark("sGARCH", "sGARCH")
benchmark_results$gjrGARCH <- fit_garch_benchmark("gjrGARCH", "gjrGARCH")
benchmark_results$eGARCH <- fit_garch_benchmark(
  "eGARCH",
  "eGARCH",
  solver = "gosolnp"
)
