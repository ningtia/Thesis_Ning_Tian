...
make_garch_spec <- function(model_name, x, fixed_pars = list(), start_pars = list()) {
  ...
  if (model_name %in% c("sGARCH", "gjrGARCH") && ncol(x) > 0L) {
    vxreg_bounds <- setNames(
      rep(list(c(-Inf, Inf)), ncol(x)),
      paste0("vxreg", seq_len(ncol(x)))
    )

    set_bounds <- getFromNamespace("setbounds<-", "rugarch")
    spec <- set_bounds(spec, vxreg_bounds)
  }
  ...
  function(train_indices, previous_model, refit_id, bench) {
    ...
    for (solver_name in solvers) {
      ...
      if (fit@fit$convergence != 0L) {
        failures <- c(failures, paste(solver_name, "optimizer convergence code",
                                      fit@fit$convergence))
        next
      }
      if (!length(coefficients) || any(!is.finite(coefficients))) {
        failures <- c(failures, paste(solver_name, "non-finite fitted coefficients"))
        next
      }

      model <- list(model_name = model_name, solver = solver_name,
                    coefficients = coefficients)
      state <- list(history_indices = train_indices)
      probe <- tryCatch(
        garch_one_step_parameters(model, state, max(train_indices) + 1L, bench),
        error = function(e) e
      )
      if (inherits(probe, "error")) {
        failures <- c(failures, paste(solver_name, "one-step forecast failed:",
                                      conditionMessage(probe)))
        next
      }
      if (!is.finite(probe$sigma_t) || probe$sigma_t <= 0 ||
          !is.finite(probe$nu_t) || probe$nu_t <= 2) {
        failures <- c(failures, paste(solver_name,
                                      "invalid one-step sigma or Student-t shape"))
        next
      }
