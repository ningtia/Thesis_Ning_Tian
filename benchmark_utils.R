...
add_lagged_covariates <- function(raw,
                                  covariates = default_benchmark_covariates()) {
  for (col in covariates) {
    raw[[paste0("lag_", col)]] <- c(NA, head(raw[[col]], -1))
  }
  raw
}
...
load_benchmark_data <- function(train_file = "train_dataset.csv",
  ...
  required_cols <- unique(c("Date", ".split", target_col, covariates))
  ...
  # The split CSVs from processing.ipynb already store x_{t-1}.
  # Keep covariates_are_lagged = TRUE for the thesis benchmark scripts.
  if (isTRUE(covariates_are_lagged)) {
    x_cols <- covariates
  } else {
    raw <- add_lagged_covariates(raw, covariates)
    x_cols <- paste0("lag_", covariates)
  }
...
make_oos_chunks <- function(bench,
  ...
  oos_indices <- if (isTRUE(include_validation)) {
    c(bench$split_rows$valid, bench$split_rows$test)
  } else {
    bench$split_rows$test
  }
  oos_indices <- sort(oos_indices)

  if (length(oos_indices) == 0) {
    return(list())
  }
